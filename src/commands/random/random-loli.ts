...
export default {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      logger.info(`Fetching Loli image for: ${interaction.user.tag}`, 'RBA');
      
      // Tambahkan parameter untuk hindari cache
      const apiUrl = `https://nvlabs.my.id/nv/random/loli?t=${Date.now()}`;
      ...
      // Buat attachment
      const filename = `loli_${Date.now()}.${extension}`;
      ...
      logger.error(`Failed to fetch Loli image: ${error}`, 'RBA');
